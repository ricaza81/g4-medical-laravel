...
function listado_todos_prospectos_comercial()
    ...
    ->with("events",$events)
     ...
     public function form_nueva_garantia()
        ...
        $cultivos=Cultivos::all();
        $zonas=Zonas::all();
        $codigos=Codigos::where('estado','desactivado')->get();
        $canales=Canales::all();
        $departamentos=Departamentos::where('id_pais','=',1)->get();
           
        
    return view('formularios.agricultor.form_nueva_garantia')
    ->with("usuario",  $usuarioactual)
    ->with("prospectos",$prospectos)
    ->with("events",$events)
        ->with("events_day",$events_day)
    ...
    public function agregar_nueva_garantia(Request $request)
    {

        $data=$request->all();

         /*  $reglas = array('usuario' => 'required|Numeric|min:1',
                           'email' => 'required',
                           'departamento' => 'required|Numeric|min:1',
                           'ciudad' => 'required|Numeric|min:1',
                           'ref_color' => 'required|Numeric|min:1',
                           'codigo' => 'required|Numeric|min:1',
                                 );
        $mensajes= array('usuario.required' =>  'Debes ingresar tu nombre',
                        
                        'departamento.numeric' =>  'Falta el departamento',
                        'ciudad.numeric' =>  'Falta la ciudad',
                        'codigo.numeric' =>  'Selecciona un codigo',
    ...
    return view('formularios.agricultor.form_nueva_garantia')
        ...
        $garantia->id_departamento  =  $data["departamento"];
        $garantia->id_ciudad  =  $data["ciudad"];
        $garantia->id_usuario  =  \Auth::user()->id;  
        $garantia->usuario  =  $data["usuario"];
        $garantia->email  =  $data["email"];             
        $garantia->ref_color  =  $data["ref_color"];
        $garantia->codigo= $data["codigo"];
        $resul= $garantia->save();

        //$pdf = \PDF::loadView('listados.garantia',compact('resul'));

        $view =  \View::make('listados.garantia', compact('resul'))->render();
        $pdf = \App::make('dompdf.wrapper');
        $pdf->loadHTML($view);
        return $pdf->stream('garantia');

        Mail::send('correo.plantilla_garantia', $data, function ($message)
            use ($result) {
        ...
        //->replyTo('laurazambranoduran.abogada@gmail.com', 'Laura Zambrano')
        ->subject('Tu garantía está activada')
        //->attachData($pdf->output(), "Factura COL-FT-".$factura->id.".pdf");
        ->attachData($pdf->output(), "Garantia Lifenmedical.pdf");
        });
